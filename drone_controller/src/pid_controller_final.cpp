...
class Controller
{
	...
	public:
		...
		Controller(ros::NodeHandle ao_nh) : po_nh(ao_nh)
		{
			...
			// Subscriber to /states topic from object_detector/Corners
			sub = po_nh.subscribe("/states", 10, &Controller::controllerCallBack, this); 
            // Landing client
            ...
            zini = 2.5; // Initial alttitude
		}

		void controllerCallBack(const object_detector::States& msg) //Callback 
		{
			...
			// Drone service for automatic landing when it reaches an specific altitude
